...
cmdline_proxy(char *arg) {
  ...
  char *proxy_port_str = strrchr(arg, ':'); /* explicit port ? */
  if (proxy_port_str) {
    //char *ipv6_delimiter = strrchr((const char *)arg, ']');
    char *ipv6_delimiter = strrchr(arg, ']');
    if (!ipv6_delimiter) {
  ...
  if ( (proxy.s = (uint8_t*)coap_malloc(proxy.length + 1)) == NULL) {
    proxy.length = 0;
    return 0;
  }
...
static int cmdline_read_hint_check(const char *arg) {
  ...
  while (fgets(tmpbuf, sizeof(tmpbuf), fp) != NULL) {
    ...
    if (tcp) {
      ...
      new_ih_list = (ih_def_t*)realloc(valid_ihs.ih_list,
...
main(int argc, char **argv) {
  ...
  while ((opt = getopt(argc, argv, "a:b:c:e:f:h:j:k:l:m:o:p:rs:t:u:v:A:B:C:J:K:H:M:NO:P:R:T:U")) != -1) {
    ...
    case 'U':
      ...
      break;
    case 'v':
      log_level = (coap_log_t)strtol(optarg, NULL, 10);
      break;
    case 'l':
